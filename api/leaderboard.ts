...
export default async function handler(request: any, response: any) {
  if (request.method === 'GET') {
    try {
      const { rows } = await pool.query('SELECT username, score FROM leaderboard ORDER BY score DESC LIMIT 10;');
      return response.status(200).json(rows);
    } catch (error: any) {
      if (error.message?.includes('relation "leaderboard" does not exist')) {
        await pool.query('CREATE TABLE IF NOT EXISTS leaderboard ( username VARCHAR(255) PRIMARY KEY, score INTEGER NOT NULL );');
